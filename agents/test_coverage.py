"""
Test coverage specialist agent — finds missing or weak tests via Groq.
"""
...
async def _review_chunk(chunk: DiffChunk) -> list[dict]:
    ...
    try:
        ...
        print(f"[test_coverage_agent] Groq API error for {chunk.filename}: {exc}")
        return []
...
async def test_coverage_agent(chunks: list) -> list[dict]:
    ...
    test-coverage prompt to Groq, and merges findings into one list.
