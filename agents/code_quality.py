"""
Code quality specialist agent — finds maintainability and style issues via Groq.
"""
...
async def _review_chunk(chunk: DiffChunk) -> list[dict]:
    ...
    try:
        ...
        print(f"[code_quality_agent] Groq API error for {chunk.filename}: {exc}")
        return []
...
async def code_quality_agent(chunks: list) -> list[dict]:
    ...
    code-quality prompt to Groq, and merges findings into one list.
